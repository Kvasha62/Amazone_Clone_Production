...
# Публичный номер отправления: SHP-{8 цифр}
# Формат: SHP-00000001
#
# F-8 (#73): это КАНОНИЧЕСКИЙ публичный идентификатор ресурса Shipment —
# он адресует отправление в URL и возвращается в payload.
# Не путать с двумя другими сущностями:
...
SHIPMENT_NUMBER_DIGITS = 8

# Совместимость: internal_tracking исторически использует тот же формат.
