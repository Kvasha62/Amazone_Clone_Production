...
* **Product** — публичный идентификатор UUID; каталог отдаёт его как ``id``
  (нормативно). Публичного поля ``uuid`` нет: два ключа с одним значением
  были бы двумя конкурирующими пространствами идентификаторов на одном
  ресурсе. Внутреннее модельное поле ``Product.uuid`` остаётся внутренним.
  Отзывы ссылаются полем ``product_id`` типа UUID (конкурирующего
  ``product_uuid`` нет).
